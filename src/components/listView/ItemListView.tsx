...
export interface ListViewEntry {
  itemName: string,
  dates: ItemInDatabase[],
}

const getExpiryDateLabelColor = (expiryDate: Date) => {
...
const RenderExpiryDateLabel = ({ expiryDate }: { expiryDate: DateNumber }) => {
  if (!expiryDate) {
    return (
      <Text
        style={{
          ...styles.labelWithBackground,
  ...
  const labelString =
    difference < 0
      ...
      : difference === 1
      ? `Expiring Tomorrow`
      : `Expiring in ${difference} days`;

  return (
    <Text
      style={{
        ...styles.labelWithBackground,
        backgroundColor: getExpiryDateLabelColor(expiryDateObject),
        color: "white",
        fontWeight: "bold",
      }}
    >
...
interface ItemListViewInterface {
  data: ListViewEntry[];
  refreshing: boolean;
  onRefresh: () => void;
  onLeftButtonPress: (i: number, j: number) => void;
  onRightButtonPress: (i: number, j: number) => void;
}
...
export default function ItemListView({
  ...
  onLeftButtonPress,
  onRightButtonPress,
}: ItemListViewInterface) {
  const [expandedItemId, setExpandedItemId] = useState(-1);
  

  const ItemView = ({ item, index: i }: { item: ListViewEntry; index: number }) => {
    return item.dates.length > 1 ? (
      ...
      <ListItem.Accordion
        ...
        content={
          <>
            <Image
              ...
              style={styles.itemListImage as ImageStyle}
            />
        ...
        onPress={() =>
          expandedItemId !== i ? setExpandedItemId(i) : setExpandedItemId(-1)
        }
      >
        <View>
