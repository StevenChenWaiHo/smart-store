...
export default function AddItemScreen() {
  ...
  const [itemStatus, setItemStatus] = useState({
    editing: false,
    scanned: false,
  }); // Combine two state to avoid duplicate triggers
  ...
  const addItemToList = async (itemData: Item) => {
    ...
    db.transaction((tx) => {
      tx.executeSql(
        "INSERT INTO list (itemName, date, quantity, image, notificationId, barcode, remarks) values (?, ?, ?, ?, ?, ?, ?)",
        [
          itemData.itemName,
          itemData.date,
          itemData.quantity,
    ...
    if (barcodeFound.from === "sql") {
      db.transaction((tx) => {
        tx.executeSql(
          "UPDATE barcodeMap SET itemName = (?), quantity = (?), image = (?) WHERE barcode = (?)",
          [
            itemData.itemName,
            itemData.quantity,
            itemData.image,
      ...
      db.transaction((tx) => {
        tx.executeSql(
          "INSERT INTO barcodeMap (barcode, itemName, quantity, image) values (?, ?, ?, ?)",
          [
            itemData.barcode,
            itemData.itemName,
            itemData.quantity,
  ...
  const handleBarCodeScanned = (barcode: BarCodeScanningResult) => {
    ...
    db.transaction((tx) => {
      tx.executeSql(
        "SELECT * FROM barcodeMap WHERE barcode = (?)",
        [barcode.data],
        (txObj, resultList) => {
          ...
          if (resultList.rows.length === 1) {
            ...
            fetch(
              `https://world.openfoodfacts.org/api/v0/product/${barcode.data}.json`
            )
    ...
    () =>
      itemStatus.editing
        ...
        ? barcodeFound.status
          ...
          : ["25%%", "100%"]
        : ["100%"],
    [itemStatus.scanned, itemStatus.editing, barcodeFound.status]
  );
  ...
  const RenderBottomSheetWhenScanned = barcodeFound.status ? (
    <View style={styles.barcodeKnownBottomSheetContainer}>
      ...
      <View style={{ flexDirection: "column", flex: 4 }}>
        <View style={{ ...styles.topLeftContainer, flex: 2 }}>
          ...
          <Text numberOfLines={1} style={styles.bottomSheetBoldText}>
            {item?.itemName}
          </Text>
        ...
        >
          <View style={{ ...styles.buttonContainer, flex: 2 }}>
            <Button onPress={handleItemEdit} title="Edit Details" />
          </View>
          <View style={{ ...styles.buttonContainer, flex: 2 }}>
            <Button onPress={() => addItemToList(item)} title="Add to List" />
          </View>
        </View>
      </View>
    </View>
      ...
      <View style={{ flexDirection: "column", flex: 4 }}>
        <View style={{ ...styles.topLeftContainer, flex: 2 }}>
          ...
          <Text style={styles.bottomSheetBoldText}>
            {item?.itemName || "Not Found in Database"}
          </Text>
        </View>
        <View style={{ ...styles.bottomRightContainer, flex: 1 }}>
          <Button onPress={handleItemEdit} title="Add Item" />
        </View>
      </View>
    </View>
  );
  ...
  async function registerForPushNotificationsAsync() {
    ...
    if (Device.isDevice) {
      ...
      if (finalStatus !== "granted") {
        alert(
          "Please allow notification. You can grant the app notification access in app settings"
        );
        return;
      }
      ...
      <View style={styles.container}>
        {focused && (
          <Camera
            style={{
              ...styles.camera,
              alignItems: takingPhoto ? "center" : "flex-end",
            }} // Camera Button Postion change when taking photo
            type={CameraType.back}
            onBarCodeScanned={
              itemStatus.scanned || itemStatus.editing
                ? undefined
          ...
          >
            ...
            <TouchableOpacity
              ...
              style={
                takingPhoto ? styles.takePhotoButton : styles.addItemButton
              }
            >
